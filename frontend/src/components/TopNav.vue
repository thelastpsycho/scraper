      ...
      <div class="flex justify-between h-14">
        ...
        <div class="flex items-center">
          <div class="flex-shrink-0 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-auto text-app-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <h1 class="text-xl font-bold text-app-tertiary ml-2">Scraper</h1>
          </div>
        </div>
        ...
        <div class="flex items-center sm:hidden">
          <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="inline-flex items-center justify-center p-1.5 rounded-md text-app-tertiary hover:bg-app-secondary focus:outline-none">
            <span class="sr-only">Open main menu</span>
            <Bars3Icon v-if="!isMobileMenuOpen" class="block h-5 w-5" aria-hidden="true" />
        ...
        <router-link v-for="item in navigation" :key="item.name" :to="item.href"
          ...
          :class="[
            route.path === item.href
              ? 'bg-app-secondary text-app-accent'
              : 'text-app-tertiary hover:bg-app-secondary/60 hover:text-app-accent',
            'block px-3 py-1.5 rounded-md text-sm font-medium flex items-center'
          ]"
        >
